...
- **`ensure_directory(directory_path, purpose)`** (from `shared_utils.py`)
  - Ensures the specified directory exists, creating it if necessary
  - Verifies write permissions (e.g., via a test file or equivalent check)
  - Raises ValueError or an appropriate exception if the directory cannot be created or is not writable

- **`create_tgz_backup(directory, output_filename)`** (line 95)
...
- **`create_backup(directory, output_dir=None)`** (line 160)
...
- **`upload_backup_to_azure(blob_service_client, container_name, backup_filepath, blob_name)`** (line 181)
...
- **`ensure_container_exists(blob_service_client, container_name)`** (line 122)
